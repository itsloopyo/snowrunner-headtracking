...
const int kKeysToName[] = {
    ...
    // The ends of the range the config layer accepts. 0x08 is the low end now
    // that the mouse buttons (0x01-0x06) are refused.
    0x08, 0x70, 0xFE,
};
...
int main() {
    std::printf("SnowRunner head tracking - hotkey name tests\n");
    std::printf("=======================================================\n");
    EveryKeyGetsAName();
    AnUnnameableKeyFallsBackToItsCode();
    return sr_test::Summary("hotkey names");
}
