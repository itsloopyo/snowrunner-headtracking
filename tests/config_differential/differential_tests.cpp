...
// pair, the mode from the pair, the yaw mode from WorldSpaceYaw, Fov as
// InstallCameraHook takes it, and each hotkey list through ParseKeyBindings and
// RegisterKeyBindings.
Record ObserveCanonical(const Config& c) {
    ...
    r["start.world_yaw"] = Flag(c.world_space_yaw);
    std::vector<sr_oracle::Registration> registrations;
    const std::pair<int, const std::string*> lists[] = {{sr_oracle::kToggle, &c.toggle_key},
                                                        {sr_oracle::kCycleMode, &c.cycle_tracking_mode_key},
                                                        {sr_oracle::kYawMode, &c.yaw_mode_key}};
...
void Compare(const std::vector<Input>& inputs) {
    const std::string committed = ReadFileBytes(fs::path(SR_SOURCE_DIR) / "HeadTracking.ini");
    const cfg::ConfigTable<Config> table = config::Table();
