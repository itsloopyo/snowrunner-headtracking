...
void DirectoryOfTests() {
    ...
    std::wstring dir;
    Check(DirectoryOf(L"C:\\Games\\SnowRunner\\SnowRunner_x64.exe", dir)
              && dir == L"C:\\Games\\SnowRunner",
          "a normal install path yields the containing directory");

    Check(DirectoryOf(L"C:\\SnowRunner_x64.exe", dir) && dir == L"C:",
          "an EXE at a drive root yields the drive");

    Check(DirectoryOf(L"\\\\server\\share\\game\\SnowRunner_x64.exe", dir)
    ...
    Check(!DirectoryOf(L"SnowRunner_x64.exe", untouched) && untouched == L"sentinel",
...
void NarrowToAnsiTests() {
    ...
    std::string narrow;
    Check(NarrowToAnsi(L"C:\\Games\\SnowRunner", narrow)
              && narrow == "C:\\Games\\SnowRunner",
...
int main() {
    std::printf("SnowRunner head tracking - exe path tests\n");
    std::printf("===============================================\n");
    DirectoryOfTests();
    NarrowToAnsiTests();
    HostExeDirectoryTests();
    return sr_test::Summary("exe paths");
}
