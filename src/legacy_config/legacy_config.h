...
// v0.2.0, the last build before the canonical config format, did, so a
...
// a3d911b, which hold v0.2.0's bytes, with three changes: it fills this frozen
...
// and it lives in namespace sr_ht::legacy. It also takes the file's full path
...
namespace sr_ht::legacy {

struct Config {
    std::uint16_t udp_port = 4242;
    bool enable_on_startup = true;

    // The angle the frame spans across its width, in degrees: 0, or 30 to 140.
    // 0 left the game's own projection alone.
